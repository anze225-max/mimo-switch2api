...
1. 本项目是**非官方第三方个人工具**，仅供学习与研究用途，与小米科技有限责任公司及其关联公司（以下统称「小米」）**无任何隶属、授权、背书或合作关系**，小米未参与本项目的开发、分发或支持。
2. 「MiMo」「小米」等商标、标识与产品名称归其权利人所有。本项目仅在指明兼容对象的必要范围内使用相关名称（指称性使用），**不使用任何小米的商标图形、图标、宣传素材作为本项目标识**；本项目不包含小米的任何代码、素材或凭证。
3. 登录通过小米官方页面由用户本人完成，账号凭证仅加密存储在用户本机，本项目不收集、不上传任何账号数据。
4. 使用者须自行确保使用行为符合小米的服务条款、开发者协议及所在地法律法规。**因使用本项目（包括但不限于账号封禁、额度调整、服务变更）产生的一切后果由使用者自行承担**。
5. 本软件按「现状」（AS IS）提供，不提供任何明示或默示担保；在适用法律允许的最大范围内，作者不对任何直接或间接损失负责。
